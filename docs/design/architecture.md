...
- **A separate service, not another pipeline stage.** Guidance helps external Agents produce valid declarations. It does not recognize declarations, parse data, or render pages. Disabled destinations incur no guidance-file access or rendering branches. Loading subscription settings does not inspect destination content.
...
- **An enabled destination authorizes full replacement.** Each destination has an explicit switch, initially off. Enabling saves that choice and writes immediately. Disabling keeps the file and stops this vault's updates. File edits do not change authorization. Standard and custom rows that resolve to the same path share one subscription.
- **Version-only scheduling.** The last successfully applied plugin version avoids repeated writes on ordinary reloads. A pending write or a different running version triggers full replacement, including recreation of a deleted destination. No content hashes, edit merging, ownership checks, downgrade protection or historical migration are needed.
- **Host capabilities stay at the boundary.** Vault operations use Obsidian APIs; Global Skills use desktop-gated filesystem operations. Guides remain vault-local. Global scope, folder and subscriptions stay in vault- and device-local storage. There is no cross-vault coordination. A temporary complete sibling protects global files from partial-write failures.
- **Persist intent before writing.** A failed state save leaves the destination alone. A failed file write leaves enablement pending for retry; success is acknowledged only after both file replacement and version persistence. A destination failure never enters the rendering pipeline.
- **Folder selection is not enrollment.** Active custom destinations lock their path field. Changing folders requires disabling first, leaving the old file in place. Changing the scope selector affects only which controls are displayed.
- **Engineering procedure:** [[docs/engineering/agent-guidance-imports|agent-guidance-imports.md]] defines state storage, write ordering, error handling and verification.
